...
function getPaymentStatus(balance, doc_id) {
    let status_content = "<b style='color: green; font-size: 90%'>Montant payé </b>";
    var edit_button = '<a id="edit-' + doc_id + '" class="btn-floating btn-small waves-effect waves-light grey" onclick="editInvoiceBalance(\'' + doc_id + '\')"><i class="material-icons left">euro_symbol</i></a>';

    if (!(balance === undefined || balance === 0)) {
        balance = balance.toFixed(2);
        status_content = `<b style='color: red; font-size: 90%'>Montant restant : </b> <div class='new chip'>${balance}</div>`;
    }
...
function retrieveFromFirestore() {
    ...
    .then(function(querySnapshot) {
        querySnapshot.forEach(function(doc) {
            ...
            var entry_data = "<td>" + invoice['invoice_number'] + "</td><td>" + invoice['emetteur_name'] + "</td><td>" + invoice['client_name'] + "</td><td>" + invoice["invoice_date"] + "</td><td>" + getPaymentStatus(invoice["invoice_balance"], doc.id) + "</td><td>" + preview_button + download_button + edit_button + send_button + delete_button + "</td>";
...
function updateInvoiceBalance(doc_id, new_balance) {
    ...
    docRef.get().then(function(doc) {
        if (doc.exists) {
            ...
            data['invoice_paid_amount'] = round2Decimals(paid);
            ...
            .catch(function(error) {
                alert("Erreur lors de la mise à jour de la balance.");
            });

        } else {
            alert("Erreur lors de la distribution des données.");
        }
    }).catch(function(error) {
...
function previewInvoice(doc_id) {
    ...
    docRef.get().then(function(doc) {
        ...
        alert("Erreur au chargement du document." );
    });
...
function exportHistoryInvoice(doc_id) {
    ...
    docRef.get().then(function(doc) {
        if (doc.exists) {
            ...
            document.getElementById("anc-" + doc_id).classList.replace("black", "green");
        } else {
            alert("Ce document n'existe pas.");
        }
    }).catch(function(error) {
...
function deleteInvoice(doc_id)  {
    var confirmation = confirm("Confirmez-vous la suppression de ce document ?");
...
function sendEmail(doc_id) {
    ...
    docRef.get().then(function(doc) {
        if (doc.exists) {
            ...
            var mail = prompt("Veuillez entrer l'adresse mail du destinataire : ", "adresse_client@hotmail.com");
            if (mail === null || mail === ""){
                alert("Aucune adresse n'a été entrée !");
            }
            else{
                var objFacture = "";
                if (data['invoice_msg']) objFacture = " - " + (data['invoice_msg']);

                var emetteur = "";
                if (data['emetteur_name']) emetteur = " - " + (data['emetteur_name']).toUpperCase();

                var montant = "";
                if (data['purchase_list'].total_ttc) montant = " - " + (data['purchase_list'].total_ttc.toFixed(2)) + " EUR";

                var objet = "Facture " + data['invoice_number'] + objFacture + emetteur + montant;

                var corps = "[Facture " + data['invoice_number'] + emetteur + "]\n\n\n" + "Bonjour,\n" +
                    "\n" +
                    ...
                    "\n                  Échéance : "+ data['conditions'] + "\n-----------------------------------------------------------------" +
                    "\n                  Total (TTC)	: " + data['purchase_list'].total_ttc.toFixed(2) + " €\n-----------------------------------------------------------------"
                ;
                ...
                window.location.href = "mailto:" + mail + "?subject=" + encodeURIComponent(objet) + "&body=" + encodeURIComponent(corps);
            }
        } else {
            alert("Ce document n'existe pas.");
        }
